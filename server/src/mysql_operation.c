...
int detectsame(MYSQL *conn,char *query,int qlen,char *srcname,int dirid)
{
	...
	for(int i=0;i<row;i++)
	{
		...
		memset(query,0,qlen);
		getnamefromid(conn,query,128,atoi(allchildid[i]),file_name);
...
int addcountfrommad5(MYSQL *conn,char *query,int qlen,char *file_md5)
{
	...
	int row=0;
	memset(query,0,qlen);
	strcpy(query,"update file_real set count=count+1 where md5='");
...
int delcountfrommd5(MYSQL *conn,char *query,int qlen,char *file_md5)
{
	...
	memset(query,0,qlen);
	strcpy(query,"select count from file_real where md5='");
	sprintf(query,"%s%s%s",query,file_md5,"';");
	puts(query);
	row=database_operate(conn,query,&result);
	printf("delcountfrommad5 row=%d\n",row);
...
int digui(MYSQL *conn,char *src,int srcid,int destid,int user_id)
{
	...
	else
	{
		...
		{
			//获取目录中第i个目录文件的类型
			gettypefromid(conn,query,128,atoi(allchildid[i]),&type);
			//获取file_name
			getnamefromid(conn,query,128,atoi(allchildid[i]),file_name);
			...
			if(type=='d')
			{
				...
				digui(conn,file_name,newsrcid,ndest,user_id);
			...
			else
			{
				...
				//获取该文件md5
				getmd5fromid(conn,query,128,atoi(allchildid[i]),file_md5);

				//获取file_size
				getsizefromid(conn,query,128,atoi(allchildid[i]),file_size);
