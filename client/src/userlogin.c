...
int UserLogin(int sfd)
{
    while (1)
    {
        ...
        //用户选择登录还是注册
        UseChoose(&account);
        // sprintf(username, "%s", account.a_name);
        // printf("%s, %s\n", account.a_name, account.a_passwd);

        //将账号密码发送给服务器
        ret = send(sfd, &account, sizeof(account), 0);
        ERROR_CHECK(ret, -1, "send_account");
        
        ret = recv(sfd, &account_1, sizeof(account_1), 0);
        ...
        if (account_1.flag == -1)
        {
            ...
            {
                printf("User Login：user name not exist !\n");
            }
            else //如果是注册失败
            {
         ...
         //根据salt 和 密码加密 账户
        char *passwd = crypt(account.a_passwd, account_1.a_passwd);
        // memset(account.a_passwd, 0, sizeof(account.a_passwd));
        sprintf(account.a_passwd, "%s", passwd);
        send(sfd, &account, sizeof(account), 0);
        // printf("已发送密码\n");
        //  printf("username = %s\n,passwd = %s\n",account.a_name, account.a_passwd);
